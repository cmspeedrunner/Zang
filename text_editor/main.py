...
def highlight_syntax(event=None):
    ...
    for keyword in ["end", "then", "to", "and","break","return","continue"]:
        ...
        while True:
            ...
            start = end
    for keyword in ["writeln", "output_file","writeln_ret", "put","msg", "passc","read","read_int", "append","pop", "opentab", "clear", "is_number", "is_string","is_list","is_function","len","extend","run", "let"]:
        start = '1.0'
        while True:
            start = text.search(r'\m{}\M'.format(keyword), start, 'end', count=stop_search, regexp=True)
            if not start:
